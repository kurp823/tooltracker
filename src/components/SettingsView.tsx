...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const [azureEndpoint, setAzureEndpoint] = useState(
    localStorage.getItem('azure_api_endpoint') || 'https://emdad-drilling-api.azurewebsites.net/api'
  );
  const [azureApiKey, setAzureApiKey] = useState(
    localStorage.getItem('azure_api_key') || 'ak_live_emdad_drilling_azure_sql'
  );
  ...
  const handleSaveAzureConfig = (e: React.FormEvent) => {
    ...
    showToast('Azure SQL connection settings saved securely to local configuration.', 'success');
  };
  ...
  const handleTestConnection = async () => {
    ...
    setTimeout(() => {
      setIsTesting(false);
      showToast('Azure SQL Database connected: Server emdad-sql-srv.database.windows.net [OK]', 'success');
    }, 1200);
  };
      ...
      <div className="bg-white border border-[#b8c9db] rounded p-4 shadow-sm space-y-3">
        <div className="border-b pb-2 flex justify-between items-center">
          <div>
            ...
            <div className="text-[11px] text-slate-500">
              Direct connection parameters for Azure SQL Database server and REST sync endpoint
            </div>
          </div>
        ...
        <form onSubmit={handleSaveAzureConfig} className="space-y-3 text-xs">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block font-bold mb-1">Azure API / Gateway Endpoint</label>
              <input
                type="url"
                value={azureEndpoint}
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="font-bold">Azure SQL Access Secret / API Key</label>
                <button
                  type="button"
          ...
          <div className="p-3 bg-slate-50 rounded border border-slate-200 text-slate-600 font-mono text-[11px] space-y-1">
            <div><strong>Host:</strong> emdad-sql-srv.database.windows.net</div>
            <div><strong>Database:</strong> EmdadDrillingOpsDB (Compatibility Level 160)</div>
            <div><strong>Authentication:</strong> Azure Active Directory Managed Identity / Key Secret</div>
          </div>
          ...
          <div className="flex justify-end space-x-2 pt-1">
            ...
            >
              {isTesting ? 'Testing Connection...' : '🔌 Test Azure SQL Connection'}
            </button>
            <button
              type="submit"
